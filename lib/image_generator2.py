...
def yolo_format_bbox(image, bbox):
    orig_h, orig_w = image.shape[:2]
    center_x = (float(bbox[0]) + float(bbox[1])) / 2 / orig_w
    center_y = (float(bbox[2]) + float(bbox[3])) / 2 / orig_h
    w = (int(bbox[1]) - int(bbox[0])) / orig_w
    h = (int(bbox[3]) - int(bbox[2])) / orig_h
...
class ImageGenerator():
    ...
    def generate_samples(self, n_samples, n_items, crop_width, crop_height, min_item_scale, max_item_scale, rand_angle, minimum_crop, delta_hue, delta_sat_scale, delta_val_scale,generate_once=False,range_of_overlay=None):
        x = []
        t = []
        for i in range(n_samples):
            ...
            for j in range(np.random.randint(n_items)+1):
                ...
                #keyboard()
                if (maximum_iou(box, boxes) < 0.3) and (maximum_BeOverlaid(box,boxes) < 0.40):
                    boxes.append(box)
